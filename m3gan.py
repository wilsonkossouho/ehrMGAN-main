...
from evaluation_metrics.visualise import visualise_gan, visualise_vae
...
class m3gan(object):
    ...
    def train(self):
        ...
        for pre in range(self.num_pre_epochs):
            ...
            # visualize
            if ((pre + 1) % self.epoch_ckpt_freq == 0 or pre == self.num_pre_epochs - 1):
                if c_rec_data_lst and d_rec_data_lst:
                    # Les listes contiennent des données, on peut visualiser
                    visualise_vae(continuous_x_random, np.vstack(c_rec_data_lst),
                                  discrete_x_random, np.vstack(d_rec_data_lst), inx=(pre + 1))
                else:
                    # Listes vides, skip la visualisation sans crasher
                    print(f"⚠️ Skipping VAE visualization at pretraining epoch {pre + 1} (empty reconstruction lists)")
        ...
        if c_real_data_lst and c_rec_data_lst and d_real_data_lst and d_rec_data_lst:
            ...
            print("✅ VAE reconstructed data saved")
        else:
            print("⚠️ No VAE reconstruction data to save, skipping np.savez")
